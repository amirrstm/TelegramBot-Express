  ...
  return new Scenes.WizardScene(
    ...
    async (ctx) => {
      try {
        ...
        const priceInfo = await orderController.getOrderPrice(userId, "buy", ctx);
    ...
    async (ctx) => {
      ...
      try {
        ...
        const orderText = orderModel.getOrderRequestText("buy", orderData.price, Number(amountText));
    ...
    async (ctx) => {
      ...
      try {
        if (ctx.update.callback_query.data === "reject_order") {
          ...
          if (order) {
            ctx.deleteMessage();
            ctx.reply(constants.orders.buySuccess, menus.mainMenu());
